...
Monitor::Monitor(CephContext* cct_, string nm, MonitorDBStore *s,
  ...
  lock("Monitor::lock"),
  timer(cct_, lock),//注意，这个lock，也让timer使用了。SafeTimer的add_event_after()都要求持有了锁。在timeout函数中，首先获取这个锁。所以在我们提供的timer callback中，都持有了锁。
  has_ever_joined(false),
  logger(NULL), cluster_logger(NULL), cluster_logger_registered(false),
  monmap(map),//在这里初始化monmap，map来自啥？参见ceph_mon.cc, main()，为monmap
  ...
  op_tracker(cct, true, 1)
{
  ...
  update_log_clients();//实际上是获取配置信息，更新clog和audit_clog的配置

  paxos = new Paxos(this, "paxos");//paxos在此创建的，是 STATE_RECOVERING 状态
  paxos_service[PAXOS_MDSMAP] = new MDSMonitor(this, paxos, "mdsmap");//dhq:paxos全部是同一个paxos实例
...
void Monitor::do_admin_command(string command, cmdmap_t& cmdmap, string format,
			       ostream& ss)
{
  Mutex::Locker l(lock);//在构造函数中获得锁，析构函数中释放锁，所以不需要显式调用加锁解锁函数。
...
{
  sanitize_options();//感觉主要是logger的一些配置变化
...
int Monitor::sanitize_options()
{
  ...
  return r;
}//疑问：到底在monitor的配置文件中，是不是写所有monitor信息？还是说各自只配置自己，互相通过通信发现？
//参见http://ceph.com/docs/master/rados/operations/add-or-rm-mons/,好像是取到的放到map-filename里面？那么正常配置是什么过程？
int Monitor::preinit()
{//preinit完成，可以接收admin cmd，比如ceph daemon mon.<id> add_bootstrap_peer_hint <peer ip>，获取peer addr，继续进行cluster创建。 http://docs.ceph.com/docs/master/dev/mon-bootstrap/
  lock.Lock();//在这加锁了，为啥？

  dout(1) << "preinit fsid " << monmap->fsid << dendl;

  int r = sanitize_options();//确保几个 time/timeout的值之间不冲突
  if (r < 0) {
  ...
  if (!has_ever_joined) {//没有join过，如果我是新加入，我的epoch是多少？ 跟别人如果不一致，那么以谁为准？
    // impose initial quorum restrictions?
    list<string> initial_members;
    get_str_list(g_conf->mon_initial_members, initial_members);//这个是配置文件？

    if (!initial_members.empty()) {
      dout(1) << " initial_members " << initial_members << ", filtering seed monmap" << dendl;

      monmap->set_initial_members(g_ceph_contex t, initial_members, name, messenger->get_myaddr(),
				  &extra_probe_peers); //进入了initial_members

      dout(10) << " monmap is " << *monmap << dendl;
      dout(10) << " extra probe peers " << extra_probe_peers << dendl;
    }
  } else if (!monmap->contains(name)) {//加入过，但是不在monmap里面. monmap什么时候初始化的？ => ceph_mon.cc的main()中，有 monmapbl.read_file
    derr << "not in monmap and have been in a quorum before; "
         << "must have been removed" << dendl;//我理解是曾经在quorum里面，后来修改了配置文件，不让再作为quorum成员了。还是网络故障被踢出去了  ? 
    if (g_conf->mon_force_quorum_join) {
      dout(0) << "we should have died but "//这里说了，died
  ...
  {//这段是清理重启前未完成的sync工作
    ...
    if (store->exists("mon_sync", "in_sync")) {//在sync_start里面设置的状态
  ...
  sync_last_committed_floor = store->get("mon_sync", "last_committed_floor");//sync_start里面设置的，应该表示在发起sync前自己的版本
  dout(10) << "sync_last_committed_floor " << sync_last_committed_floor << dendl;

  init_paxos();//包括各种paxos service，这里能看出点很多实例都是它创建的，paxos，各种 paxosservice, health monitor
  health_monitor->init();

  if (is_keyring_required()) {//kingring和fsid，都是在mkfs前准备的，用tool生成的。
  ...
  admin_hook = new AdminHook(this);//用于下面注册一些命令的钩子处理函数
  AdminSocket* admin_socket = cct->get_admin_socket();

  // unlock while registering to avoid mon_lock -> admin socket lock dependency.
  lock.Unlock(); //socket也有锁，在register_command内部会加解锁，避免两个锁冲突。但是觉得这里面的锁好奇怪，在此之前持有了lock的目的？保护store操作？ 如果在我们注册之后，还没有lock之前有命令到来，什么结果？
  ...
  g_conf->add_observer(this);//g_conf也是受lock的保护？ 估计如此，没别的全局锁了。

  lock.Unlock();
  return 0;
}

int Monitor::init() //在调用preinit()和init()之间，调用了msgr->start().
{
  dout(2) << "init" << dendl;
  lock.Lock();
  ...
  // i'm ready!
  messenger->add_dispatcher_tail(this);//加了dispatcher就可能开始干活，所以得hold了lock
...
{
  if (paxos->is_writing() || paxos->is_writing_previous()) {//在这两个状态下，是后台异步完成commit再修改为其他状态，此时paxos/paxos service上下文已经放了锁。其他的状态修改，应该都是中间不放锁的。
...
//这个函数在正常加载时，应该也会调用，统一处理
void Monitor::bootstrap()//wait_for_paxos_write 需要持有lock，但是调用bootstrap时，基本都是超时之类的情景，如何获取了lock?
{//在dispatch时，已经持有锁。如果是timeout处理，timer已经获得了锁指针。在处理函数中加锁保护。
  dout(10) << "bootstrap" << dendl;
  wait_for_paxos_write();

  sync_reset_requester();
  unregister_cluster_logger();
  cancel_probe_timeout();//里面会重新开始probe过程，所以取消timeout

  // note my rank
  int newrank = monmap->get_rank(messenger->get_myaddr());//monmap这个时候啥状态？
  if (newrank < 0 && rank >= 0) {
  ...
  if (newrank != rank) {//换了rank，先down所有的连接。但是这个在什么情况下发送的不知道
  ...
  // reset
  state = STATE_PROBING;//如果后面进入election，则修改为STATE_ELECTING

  _reset();//会调用paxos->restart()，将其状态修改为 recovering。执行一系列的clear/reset之类操作
  ...
  // singleton monitor?
  if (monmap->size() == 1 && rank == 0) {//这个monmap，怎么保证正确？ 如果我的monmap跟别的节点不同，那是什么结果？ 我理解就是信任，非拜赞庭
    win_standalone_election();//rank不等于0，则不包含自己。怎么处理？
    return;
  }

  reset_probe_timeout(); //设置了一个timeout，是不是各个monitor都要在这个时间响应？ 那么大家也都是起来了。
  //http://docs.ceph.com/docs/master/dev/mon-bootstrap/，如果没有 add_bootstrap_peer_hint 选项给晚了，是不是通过超时来等待输入，再完成后续工作？
  // i'm outside the quorum
  if (monmap->contains(name)) //在monmap，但是不在 quorum 里面
    outside_quorum.insert(name); //outside用来统计多少个人reply了probe，凑齐了quorum个就开始election.
  //发起probe，当probe回应的个数达到quorum时，即可启动election。也就是先probe，起码知道有多少个是alive的，后再election产生leader。
  // probe monitors
  dout(10) << "probing other monitors" << dendl;
  for (unsigned i = 0; i < monmap->size(); i++) {//这个函数干的活非常多，需要probe各个monitor.
    if ((int)i != rank)
  ...
  for (set<entity_addr_t>::iterator p = extra_probe_peers.begin();
       ...
       ++p) {
    if (*p != messenger->get_myaddr()) {//自己不能probe，因为已经加入了 outside_quorum
      entity_inst_t i;
...
bool Monitor::_add_bootstrap_peer_hint(string cmd, cmdmap_t& cmdmap, ostream& ss)
{
  ...
  if (is_leader() || is_peon()) {//这个命令就是帮助bootstrap阶段找到peer的，正常运行不需要这个
...
void Monitor::_reset()
{
  ...
  for (vector<PaxosService*>::iterator p = paxos_service.begin(); p != paxos_service.end(); ++p)
    (*p)->restart(); //各个paxosservice，都调用了restart
...
void Monitor::sync_start(entity_inst_t &other, bool full)
{
  ...
  state = STATE_SYNCHRONIZING; //先修改为sync状态
  ...
  sync_reset_timeout();
  //根据是否full的sync，给出参数。这个应该才是真正的请求对方发送内容。
...
void Monitor::sync_finish(version_t last_committed)
{
  ...
  if (sync_full) {//非full不修改last_committed? 
    ...
    paxos->read_and_prepare_transactions(tx, sync_start_version,
					 last_committed);//这个函数里面，好像把[sync_start_version, last_committed]之间的全部打包进入了tx
  ...
  t->erase("mon_sync", "last_committed_floor"); //为什么删除这个？ 
  store->apply_transaction(t);
 ...
 */ //注意，probe，这个是monitor的活，不是用来选举的。跟elector.cc中的handle_propose不要混淆，那个propose是选举相关。
void Monitor::handle_probe_probe(MonOpRequestRef op)
{
  ...
  if (!is_probing() && !is_synchronizing()) {
    ...
    // it is possible we pass those checks (we overlap with //这里所说的情况，是不是还是有发生问题的可能性？ 这个属于啥情况？
    // quorum-to-be) but fail to join a quorum before it moves past
    // us.  We need to be kicked back to bootstrap so we can
    // synchonize, not keep calling elections.  //not keep calling elections，什么意思？synchonize的关键点在哪？
  ...
  r->name = name;
  r->quorum = quorum;
  monmap->encode(r->monmap_bl, m->get_connection()->get_features());//我的monmap发过去
  r->paxos_first_version = paxos->get_first_committed();//把我的版本first_committed和last_committed都发送过去，让对方自己判断该怎么办。
  r->paxos_last_version = paxos->get_version();
  m->get_connection()->send_message(r);

  // did we discover a peer here?
  if (!monmap->contains(m->get_source_addr())) {//遇到新来的，直接添加进extra_probe_peers？ 
...
void Monitor::handle_probe_reply(MonOpRequestRef op)
{
  ...
  // discover name and addrs during probing or electing states.  这句解释不太懂
  if (!is_probing() && !is_electing()) {//bootstrap的probe走这个流程很明确，为什么会是is_electing()? 状态怎么来的？我发出的probe，别人很久才回复，但是我已经开始elect了？
    return;//因为handle_proble_reply里面，攒够了回复后，就开始election了。
  }
  ...
  if (!mybl.contents_equal(m->monmap_bl)) {//检查大家的momap是否一致，如果别人比自己新，那么就换成别人的，继续bootstrap
    ...
    if (m->has_ever_joined && (newmap->get_epoch() > monmap->get_epoch() ||
      ...
      bootstrap();//这里又调用bootstrap，感觉层次比较奇怪
      return;
    }
    delete newmap;
  }
  ...
  if (monmap->get_epoch() == 0 && peer_name.find("noname-") == 0) {//注意，get_epoch返回了0，代表刚开始干活？互相不知道名字，只知道ip?
    dout(10) << " renaming peer " << m->get_source_addr() << " "
	     << peer_name << " -> " << m->name << " in my monmap"
	     << dendl;
    monmap->rename(peer_name, m->name);//原来是noname，现在message中包含了新名字

    if (is_electing()) {//中断选举，bootstrap
      bootstrap();
      return;
    }
  } else {
  ...
  // new initial peer?  epoch==0的，是新加入的pper
  if (monmap->get_epoch() == 0 &&
      monmap->contains(m->name) &&
      monmap->get_addr(m->name).is_blank_ip()) {//原来没有peer的ip，现在有了，修改ip，再次bootstrap
  ...
  // end discover phase
  if (!is_probing()) {//注意，非probing才返回, probe继续往下走
    return;
  }
  ...
  //疑问：这里没有判断elector持久化的epoch，而是判断paxos的version等。为什么
  if (m->paxos_last_version < sync_last_committed_floor) {
	...
	m->paxos_first_version > 1) {  // no need to sync if we're 0 and they start at 1.
      dout(10) << " peer paxos versions [" << m->paxos_first_version
	       ...
	       << dendl;
      cancel_probe_timeout();
      sync_start(other, true);//完全同步，这些都是基于信任原则
      return;//注意，到此返回了。在sync_finish()完成后，继续进行init
    }
    if (paxos->get_version() + g_conf->paxos_max_join_drift < m->paxos_last_version) {
      dout(10) << " peer paxos version " << m->paxos_last_version
	       ...
	       << dendl;
      cancel_probe_timeout();
      sync_start(other, false);//差距不太大，在窗口内。即在对方的(paxos_first_version和paxos_last_version之间，可以部分同步。
      return;//注意，到此返回了
    }
  }

  // is there an existing quorum?
  if (m->quorum.size()) {//他们已经形成一致，存在了一个quorum
    ...
    if (monmap->contains(name) &&
        !monmap->get_addr(name).is_blank_ip()) {//其他的节点构成了一个quorum，并且认为我应该在里面，那么只有发起election，让我进去.
      // i'm part of the cluster; just initiate a new election
      start_election();//启动election，在这个过程中，还会收到本轮尚未残留的OP_REPLY? 解释了前面所说的is_electing()?
    } else {
    ...
    if (monmap->contains(m->name)) {//对方在我的monmap里面，但是对方还没有形成quorum，放到outside_quorum里面存着
    ...
    if (outside_quorum.size() >= need) {//如果自己不在outside_quorum里面，说明不是自己发起的probe? 
      if (outside_quorum.count(name)) {//自己什么时候加入outside_quorum? ==> bootstrap里面，直接把自己插入了outside_quorum 
        dout(10) << " that's enough to form a new quorum, calling election" << dendl;
        start_election();//足够的peer已经响应，可以做election了。
      } else {
...
void Monitor::join_election()//这个函数进来时hold了lock? 因为wait_for_paxos_write直接unlock
{
  dout(10) << __func__ << dendl;
  wait_for_paxos_write();//也是需要等待和reset paxos
  _reset();//可能正在做probe什么的，废除，直接加入选举
  state = STATE_ELECTING;

  logger->inc(l_mon_num_elections); //我只是加入选举，调用者会处理给发起election的ack(Elector::defer)
}//start_election 和 bootstrap(), 都调用_reset，到底什么区别？ 我理解在bootstrap本身不会调用start_election，只是发起PROBE，
//但是在handle_probe_reply()，完成前期准备后，会调用start_election
void Monitor::start_election()
{
  dout(10) << "start_election" << dendl;
  wait_for_paxos_write();
  _reset();
  state = STATE_ELECTING; //这个mon的状态，不是paxos的。疑问：我进入了选举态，别人需要进入吗？怎么进入？handle_propose => bump_epoch =>join_election
  ...
  elector.call_election();//在现有的实现里面，这个函数就是调用Elector::start()
}
...
void Monitor::win_standalone_election()
{
  ...
  assert(rank == 0);
  set<int> q;
  q.insert(rank);//只插入自己，实际上会作为新的quorum
                           ...
                           const set<int> *classic_monitors)
{
  ...
  // round without agreeing on who the participants are. //我理解，这个跟 MonmapMonitor::encode_pending函数有关，但是什么时候调用的呢？
  monmon()->election_finished();  //首先是monmon，然后是其它的paxos service子类(monmon也是paxosservice子类)。bootstrap case中，到底马上就propose monmap change?
  for (vector<PaxosService*>::iterator p = paxos_service.begin();
       p != paxos_service.end(); ++p) {
    if (*p != monmon())//因为上面已经执行过了。 直接把PAXOS_MONMAP定义为0多好？不用啰嗦了。
      (*p)->election_finished();
  }
...
//对于要做update的请求，forward给leader，并且能够将reply route回去。只读的应该能处理
...
void Monitor::_ms_dispatch(Message *m)//调用前已经持有了lock，也就是说在处理各种消息(包括管理、OSD状态变更等)时，已经hold了lock
{
  if (is_shutdown()) {
    return;
  }
...
void Monitor::dispatch(MonOpRequestRef op)//由于_ms_dispatch执行时持有了lock，这个函数也是持有lock的
{
...
void Monitor::dispatch_op(MonOpRequestRef op)//由于_ms_dispatch执行时持有了lock，这个函数也是持有lock的
{
  ...
  switch (op->get_req()->get_type()) {
    ...
    case CEPH_MSG_AUTH:
      ...
      paxos_service[PAXOS_AUTH]->dispatch(op);//调用相应monitor的dispatch函数
      break;
  ...
  switch (op->get_req()->get_type()) {
    ...
    case MSG_MON_PAXOS:
      {
        op->set_type_paxos();
        MMonPaxos *pm = static_cast<MMonPaxos*>(op->get_req());//paxos通信，走的这个MMonPaxos结构体
        if (!op->is_src_mon() ||
        ...
        // sanitize
        if (pm->epoch > get_epoch()) {//非常重要： 以collection为例，如果A当选后，别人B当选了，而A不知道。如果不进行这个检查，那么A的collection全部都会成功，但是实际上应该失败了。
          bootstrap();//有了这个检查，一旦发生重新选举，那么paxos消息都会因为epoch不对而被拒绝。不会被当作正常状态去处理
          break;
        }
        if (pm->epoch != get_epoch()) {//实际上是比我的epoch小，说明可能又发生了新的选举；本消息属于上一epoch，发送消息的leader可能退位了，所以不理了。
          break;
        }
    ...
    case MSG_MON_ELECTION:
      ...
      if (!is_probing() && !is_synchronizing()) {
        elector.dispatch(op);//直接调用elector的函数
      }
      break;
...
{
  ostringstream ss;//没有snprintf，只能这么转，麻烦
  ss << monmap->get_fsid() << "\n";
  string us = ss.str();
 ...
 */
int Monitor::mkfs(bufferlist& osdmapbl)//这个里面没看到网络操作，是不是ssh到各个节点，然后分别执行相同工作？
{//命令行：ceph-mon --mkfs -i {hostname} --monmap /tmp/monmap --keyring /tmp/ceph.mon.keyring
  MonitorDBStore::TransactionRef t(new MonitorDBStore::Transaction);
