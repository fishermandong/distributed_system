...
/*摘自网页：http://docs.ceph.com/docs/v0.94/rados/operations/add-or-rm-mons/
Ceph clients and other Ceph daemons use ceph.conf to discover monitors. However, monitors discover each other using the monitor map, not ceph.conf
Since monitors use monmaps for discovery and they share monmaps with clients and other Ceph daemons, the monmap provides monitors with a strict guarantee that their consensus is valid.
monitor map是monitors之间互相发现的依据。签名说Ceph Daemons通过ceph.conf找discover monitors，又说share monmaps给clients，到底ceph.conf有用没？
If monitors discovered each other through the Ceph configuration file instead of through the monmap, it would introduce additional risks because the Ceph configuration files aren’t updated and distributed automatically. 
monmap自动被传播，但是ceph.conf不会这样
*/
...
public:
  ...
  set<entity_addr_t> extra_probe_peers;//我理解，是不在配置文件中的，但是别人probe了自己，或者extra_probe_peers设置的，就记在这个里面了，下次一块probe
...
public:
 ...
 private:
  ...
  bool ms_dispatch(Message *m) {
    lock.Lock();//重要： 这个函数一进来就获取了lock，这就让选举没法获得锁了？
    _ms_dispatch(m);
    lock.Unlock();
    return true;
  }
