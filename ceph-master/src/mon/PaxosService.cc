...
bool PaxosService::dispatch(MonOpRequestRef op)//注意这个是父类，具体每个monitor才会产生实例，而不是这个类。所以都是子类的实例在执行此函数。
{
  ...
  if (m->rx_election_epoch && //这个就在handle_forward()里面赋值了，该函数还没看
  ...
  //读操作是可以peon直接处理的，下面的对于其他请求，才判断是否leader.
  // preprocess
  if (preprocess_query(op)) //布尔型函数，返回成功，表名已经处理完了那些只读的请求
    return true;  // easy!

  // leader?
  if (!mon->is_leader()) {//走到这里，就是写了。非leader不能做，只能forward给leader
  ...
  // update
  if (prepare_update(op)) {//prepare_update对于不同的monitor, 不同的事件都是不同的，参见OSDMonitor中的实现
    double delay = 0.0;
...
void PaxosService::refresh(bool *need_bootstrap)//所谓refresh，很大程度上是因为刚刚commit导致了store被修改，这里从store再去load/get一些值
{
  // update cached versions  这个为什么需要缓存 ? 不能直接用paxos->first_committed? 实际上这是本service的 first_committed，不是 paxos的，不在一个层面
  cached_first_committed = mon->store->get(get_service_name(), first_committed_name);//根据service/monitor的名字，每一个都是不同的存储名字空间
  ...
  update_from_paxos(need_bootstrap);//具体monitor 的钩子函数
}
...
//conversion_first 到  cached_first_committed 之间的版本被删除。
...
void PaxosService::propose_pending()
{
  ...
  assert(have_pending);//这几个在调用本函数前已经确认完成了，这里确认下
  assert(!proposing);
  ...
  encode_pending(t);//这个是在父类调用的， 对于 MonmapMonitor 来说，是不是这个函数实际上就是对monmap进行打包，提交给paxos? 
  have_pending = false;
  ...
  // apply to paxos
  proposing = true;//这个实际上也是做互斥的，在C_Committed::finish()函数中，再赋为false
  paxos->queue_pending_finisher(new C_Committed(this));//设置callback，在paxos完成commit后？
  paxos->trigger_propose();//这个会调用paxos::propose_pending
}
...
void PaxosService::election_finished()
{
  ...
  finish_contexts(g_ceph_context, waiting_for_finished_proposal, -EAGAIN);//最后一个参数是-EAGAIN，应该会重来 ?  什么时候需要这样 ? 
...
void PaxosService::_active()
{
  ...
  if (mon->is_leader() && is_active()) {//必须leader才能去做propose
    ...
    } else if (!is_active()) {//前面不是检查这个并且return了吗 ? 怎么又判断 ? 
  ...
  finish_contexts(g_ceph_context, waiting_for_finished_proposal, 0);//waiting_for_finished_proposal上的各个，全部执行complete函数
  ...
  if (is_active())
    on_active();//虚函数，这个是给各个具体的monitor 的钩子函数
}
