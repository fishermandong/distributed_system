...
void MonMap::decode(bufferlist::iterator &p)
{
  ...
  ::decode_raw(fsid, p);
  ::decode(epoch, p);//encoding.h 中有些是template定义的
  if (struct_v == 1) {
...
int MonMap::write(const char *fn) 
{
  // encode
  bufferlist bl;
  encode(bl, CEPH_FEATURES_ALL);//整个对象的各个成员都打包
...
int MonMap::build_from_host_list(std::string hostlist, std::string prefix)
{
  ...
  if (parse_ip_port_vec(hostlist.c_str(), addrs)) {//从字符串提取地址信息, 应该只解析ip格式的
    for (unsigned i=0; i<addrs.size(); i++) {
      char n[2];
      n[0] = 'a' + i;//第0个，命名为a，其他用 b, c, d等类推
      n[1] = 0; //'\0'
      if (addrs[i].get_port() == 0)
	addrs[i].set_port(CEPH_MON_PORT);//默认的port
      string name = prefix;
      name += n; //后缀加到字符串前缀后面
      if (!contains(addrs[i]))
	add(name, addrs[i]);
  ...
  char *hosts = NULL;
  hosts = resolve_addrs(hostlist.c_str());//先转换成IP格式　? 
  if (!hosts)
    return -EINVAL;
  ...
  for (unsigned i=0; i<addrs.size(); i++) {//重复代码，写得次
    char n[2];
    n[0] = 'a' + i;
    n[1] = 0;
    if (addrs[i].get_port() == 0)
...
{ //这个函数，整体感觉很低效
  ...
  unsigned i = 0;
  while (i < size()) {//遍历monmap的成员，如果不在initial_members中，则移除，放到removed里面
    string n = get_name(i);//mon initial members是可以在配置文件写的，http://docs.ceph.com/docs/v0.94/rados/configuration/mon-config-ref/，按照解释，initial不是必须在的，加速进程
  ...
  //initial_members中的成员，如果不在monmap里面，加进去
