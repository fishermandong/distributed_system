...
class PaxosService {
 ...
 protected:
   ...
   * for that purpose.
   */
  version_t service_version;//°´ÕÕÉÏÃæµÄ×¢ÊÍ£¬Õâ¸ö±äÁ¿ÊÇÀúÊ·²úÎï£¬¿ÉÒÔ²»ÒªÁËÊµ¼ÊÉÏ¡£
...
public:
   ...
   */
  virtual void update_from_paxos(bool *need_bootstrap) = 0; //ÒÔmonmapÎªÀý£¬paxosºÍmonmapmonitor·Ö±ð±£´æÁËmonmap£¬µ«ÊÇepoch¿ÉÄÜ²»Í¬
   ...
   */ //PaxosService::_activeÀïÃæÓÐ×¢ÊÍ£¬ËµÁ½´Î¡£µ«ÊÇÎªÉ¶ÄØ£¿
   ...
   */
  virtual void upgrade_format() { } //ÐÞ¸Ä´ÅÅÌÊý¾Ý¸ñÊ½£¿ ÒòÎªÉý¼¶ÏµÍ³£¿
 ...
 public:
   ...
   */
  bool is_active() { //ÎÒÀí½â£¬ÊÇcan_proposeµÄÒâË¼¡£²»ÊÇËµÊÇ·ñactive.
    return
      !is_proposing() && //ÎªÊ²Ã´ is_updating Ëãactive£¬µ«ÊÇis_updating_previous²»Ëã ? 
      (paxos->is_active() || paxos->is_updating() || paxos->is_writing());
  }

  /**
   * Check if we are readable.
   * ÎÒÀí½â£¬Õâ¸ö²»ÊÇ±£Ö¤¶Áµ½×îÐÂµÄÖµ£¬µ«ÊÇÒª±£Ö¤£¬±ÈÉÏ´Î¸Ãclient¶Áµ½µÄÐÂ¡£±È½ÏµÄÓ¦¸ÃÊÇÉÏ´Î¸Ãclient¿´µ½µÄ°æ±¾¡£
   * This mirrors on the paxos check, except that we also verify that
   *
   *  - the client hasn't seen the future relative to this PaxosService  //Õâ¸öÊÇËµ£¬Ö»ÒªclientÒ»¸ö¶¼Ã»¿´µ½Ä³¸öÐÂ¶«Î÷£¬¾ÍËã²éÑ¯²»°üº¬¸ÃÐÂ¶«Î÷£¬Ò²ÎÞËùÎ½£¿
   ...
   */
  bool is_write_ready() {
    return is_active() && have_pending;//Ó¦¸ÃÊÇËµ£¬ÓÐÁËpending£¬²ÅÓÐµÄ¿ÉÒÔwriteµÄ
  }
...
//Õû¸öº¯Êý£¬ÏÈÅÐ¶Ï±¾paxoserviceÊÇ·ñÕýÔÚpropose£¬È»ºóÔÙÅÐ¶ÏpaxosÊÇ·ñactive, ¿ÉÐ´
    if (is_proposing())//paxosservice²ãº¯Êý£
      wait_for_finished_proposal(op, c);//µÈÇ°Ò»¸öproposeÍê³Éºó£¬ÔÙÖ´ÐÐ
    else if (!is_write_ready())
      wait_for_active(op, c);//¾ÍÕâ¸öµØ·½À´Ëµ£¬wait_for_activeÄÚ²¿µÄ²¿·Ö¹¤×÷¸úÉÏÃæµ÷ÓÃwait_for_finished_proposalÖØ¸´
    else
      paxos->wait_for_writeable(op, c);//µ÷ÓÃpaxosÀàµÄº¯Êý
  }
