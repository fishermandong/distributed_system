...
void MonmapMonitor::update_from_paxos(bool *need_bootstrap)
{
  ...
  dout(10) << "update_from_paxos got " << version << dendl;// 为什么说from paxos? 被更新的是mon->monmap，从mon->store读取的，这个是因为cache的原因 ? 
...
{
  pending_map = *mon->monmap;//整体copy过来了monmap
  pending_map.epoch++;//不同的monitor，这个create_pending差别很大。
