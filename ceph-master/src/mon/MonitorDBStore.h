 ...
 public:
  ...
  struct C_DoTransaction : public Context {
    ...
    void finish(int r) {
      ...
      oncommit->complete(ret); //dhq: 这个对应 queue_transaction 的参数oncommit，complete里面调用finish
    }
  };
  ...
  void queue_transaction(MonitorDBStore::TransactionRef t,
			 Context *oncommit) {
    io_work.queue(new C_DoTransaction(this, t, oncommit));//参见finisher.c的finisher_thread_entry()，负责完成io_work上的任务
  }
