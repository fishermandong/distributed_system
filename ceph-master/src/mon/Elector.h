...
class Elector {
 ...
 private:
  ...
  /**
   * Latest epoch we've seen.//看到的最新的epoch，这个东西的含义是什么？这个选举过程，不是paxos那几步的内容。
   *
   * @remarks if its value is odd, we're electing; if it's even, then we're
   *	      stable.//奇数，表明在选举，偶数，表明是稳定状态。啥含义 ? 
   */
  epoch_t epoch;//我理解，epoch就是选举用的，跟那几个version没有直接关系。
   ...
   */
  bool participating;//一般都是true，只有上面说的某些命令才导致为false. 
   ...
   */
  int	    leader_acked;//那个我们已经ack的(潜在leader?)
  /**
   ...
   * @post  epoch is an odd value  //表示正在选举，epoch变量有注释
   ...
   *
   * @pre   @p who outranks us (i.e., who < our rank) //明确：rank小的厉害
