 ...
 *dhq: bufferlist保存了一系列命令，作为一个transaction，应该就是Tx_foo_bufferlist.
 *  Tx_Foo
 ...
 *
 *  Tx proposed_tx;
 *  proposed_tx.decode(Tx_foo_bufferlist);//把Tx_foo_bufferlist里面的操作，记录到proposed_tx ?
 *
 *  Tx our_tx;
 *  our_tx.put(paxos, last_committed, 5);//dhq: 注意这个例子，paxos的version是5，而foo的版本是3，是否因为paxos上面可以有多个monitor? 
 *  our_tx.put(paxos, 5, Tx_foo_bufferlist);
 *  our_tx.append(proposed_tx);//既然Tx_foo_bufferlist已经作为我不需要理解的内容保存，为什么还需要append tx? 什么含义？
 *
 *  store_apply(our_tx);
 *
 * And the store should look like this after we apply 'our_tx':
 *dhq: foo是个什么玩意？ ==>http://ceph.com/dev-notes/cephs-new-monitor-changes/，从语义上来说，paxos位于各种monitor(这里的foo替代)之下，但是对于使用的k/v store，仅仅是不同的namespace的存储而已。
 *  paxos:
 *    first_committed -> 1
 *     last_committed -> 5
 *		    1 -> value_1
...
// i am one state machine.   重要： 与标准Paxos的差异
/**
 * This libary is based on the Paxos algorithm, but varies in a few key ways:
 *  1- Only a single new value is generated at a time, simplifying the recovery logic.  估计跟必须有leader有关？leader对于一个version只提供一个value
...
class Paxos {
   ...
   */
  Monitor *mon;//dhq: 这个monitor不是 MDSMonitor 等的父类，否则就有问题了，因为只运行一个paxos实例。那些实际monitor的父类是PaxosService.
...
public:
  ...
  enum {
     ...
     */
    STATE_RECOVERING, //dhq: leader_init() 或者 peon_init()设置，即刚当选或者落选后设置为此状态。
    /**
     * Leader/Peon is idle, and the Peon may or may not have a valid lease.
     */ //dhq: 为什么把active解释称idle? 我理解是说没活干的对于应用来说才是active，即当前不忙。
    STATE_ACTIVE,
    /**
     ...
     */
    STATE_UPDATING_PREVIOUS,//对应handle_last()里面的begin()，是对于那些有pending value的。
    /*
     ...
     */
    STATE_WRITING_PREVIOUS, //参见STATE_UPDATING_PREVIOUS的注释
    // leader: refresh following a commit
    STATE_REFRESH,//leader专用的这是
  };
...
public:
   ...
   * previous quorum  明确说了，是 previous quorum
   */
  bool is_updating_previous() const { return state == STATE_UPDATING_PREVIOUS; }

  /// @return 'true' if we are writing an update to disk
  bool is_writing() const { return state == STATE_WRITING; }//dhq: must understand state is STATE_WRITING
...
private:
   ...
   */
  version_t last_committed;//目前看到的该变量修改点，就是 commit_finish，做了 last_committed++
  /**
   ...
   *///我理解，这个只在init，recovery(collect/handle_collect)函数修改，因此不发生leader当选的情况下，应该是不变的。变化的是last_committed等几个。
  version_t accepted_pn;//现在理解是，只要接受某个leader的proposal，就接受它的一系列version/value，直到有新的proposal被接受为止。
  /**
   ...
   */  //dhq: 我理解是leader上的结构，从各个Peon收集的first_committed信息
  ...
  // active (phase 2)  dhq: 应该是active状态才使用的变量
  /**
   ...
   * not be extended. 
   */
  utime_t lease_expire;//TODO: 怎么解决各个节点间时间不一致问题？
  /**
   ...
   *dhq: 下面说的"the requested version is greater than our last committed version"，这是个什么含义？应用定义的版本？
   ...
   * whole quorum.
   */
  unsigned   num_last; //一共收到了几个对应collect phase的回应
  /**
   ...
   */
  version_t  uncommitted_v; //这几个uncommitted_*，应该都是recovery(collect/last/share)用的，平时不用它。
  /**
   ...
   *dhq: 上面一句说的，已经收到了相应的accept replies，但是没来得及commit。意思明白，但是 leader在recovery时怎么找到这个？问各个Peon?
   ...
   */
  Context    *collect_timeout_event; //dhq: 这里叫做collect_timeout_event,  phase2叫做 accept_timeout_event
  /**
   * @}
   */
   ...
   */
  MonitorDBStore::TransactionRef pending_proposal;//可以加多个operations上去？
...
private:
   ...
   * @{
   */
  /**dhq: collect过程的目的，下面说的是recovery，要对后续所有instances做phase1操作。
   * Create a new Proposal Number and propose it to the Peons.
   * dhq: 对于上一任未完成的，handle_last()还要走phase 2.
   ...
   *		  Proposal Number for the Recovery Phase.  dhq: 根据规则，在上次pn的基础上，产生新的pn
   */
  void collect(version_t oldpn);//感觉collect, last这些都不是标准的paxos过程。后面的begin, accept也都不是标准的paxos过程，步骤上不对应。
  /**
   * Handle the reception of a collect message from the Leader and reply
   * accordingly.
   * dhq: version，是不是对应paxos原来论文中的instance? 
   ...
   * dhq: handle_last函数，leader Peon 对于collect (recovery)的回答后走到这个地方。根据回应消息类型判断走到这个函数。
   ...
   * scenarios: if the replied Proposal Number is equal to the one we proposed,   //dhq: 这段解释很重要，是不是把那些已经被propose过，但是未commit的version给commit掉，然后leader就可以正常干活了？last_commit就成了下一个版本？
   ...
   * proposal in order to retry with the Proposal Number specified by the Peon.//用返回的较大的PN retry? 这个跟Paxos论文不同，那里说的是用返回的v?
   ...
   */
  void collect_timeout();//dhq: leader在collect发送timeout时，发起election，还有其他时机，或者由Peon发起election吗？
  /**
   * @}
   */
   ...
   * @{ 重要： 这里明确说了跟paxos协议的对应关系。 但是为什么begin对应 prepare 和 accept 两个？ prepare发的proposal number, accept带的是value，怎么一次搞定的？ "paxos made simple"一文，应该说过这两个合一块的问题？但是那时候没说有leader.
   */ //为什么这么简单对应完了？ 因为有了leader?   在handle_begin里面，严格要求accepted_pn == begin->pn，这就相当于保证了没有发生选举，从而不可能有别人在propose? 
  /**
   * Start a new proposal with the intent of committing @p value. //dhq: 上面说了，we can easily map Paxos::begin to both the Prepare and Accept Request phases
   *
   ...
   */ /*"paxos made code"一文说：Phase 2a: Instance 1 is ready with no value in it. The proposer sends an accept message containing the previously used ballot 201 and the value v received from
the client. 看起来，其ballot number，就是对应这里的accepted_pn?  那么就是说，多个要propose的版本(instance)，共用一个proposal number? 这个理论上是可以的*/
  void begin(bufferlist& value);  //★★★★★★★在多次执行"paxos made simple" 一文中Phase 1 (page 5) 和Phase 2时，用同一个proposal number。 注意，accept_timeout直接重新发起选举，不是尝试继续什么的。
  /** dhq: handle_begin并没有处理version相关内容，而是处理Proposal Number相关内容，version应该是隐含的instance id
   ...
   * accept it and accordingly reply to the Leader.  这里所说的high numbered proposal，具体对应的number是什么? pn
   *
   * @pre We are a Peon
   * @pre We are on STATE_ACTIVE
   ...
   *
   * @pre We are the Leader
   * @pre We are on STATE_UPDATING  dhq: 这里可以看出状态转换的一些含义，在什么状态下，接收什么，然后变成什么状态
   ...
   *///注意，在超时情况下，直接发起election，这个跟标准的paxos不同。
  void accept_timeout();
  /**
   * @}
   */
//commit本身不在两阶段协议里面，这里引入commit机制，是为了让peon们知道，这些instance/version已经被choosen。便于换leader后，只需要对少量uncommitted version重新投票。
   ...
   * 取消lease，因为lease是跟committed_version关联的。但是没找到cancel lease的代码，commit_finish()中有一句，注释为cancel: lease_expire = utime_t();
  ...
  /** 注意，在handle_begin()内部，有lease_expire = utime_t(); 即让lease失效。这个也是个自然的方法，表示要处理新的修改。所以lease的有效期，是上次commit/extend_lease，到下次begin()之间。
   * Extend the system's lease.
   *dhq: every quorum member，是指全局，还是majority? message的send的对象，也是quorum member，一致。
   ...
   * @post Fire up all the callbacks waiting for readable iif we are readable //dhq: Peon也可以处理读请求？ 什么条件下可以？
   ...
   *dhq: 为什么这个情况下需要election? ==>peon不回复，说明我这个老大的位置坐不住了。
   * @pre We are the Leader
   * @pre We are on STATE_ACTIVE
...
public:
   ...
   *	   or on STATE_RECOVERING otherwise.//dhq: 正常情况下变成STATE_RECOVERING， quorum为1我认为是非正常配置
   */
  void leader_init();
  /**
   ...
   *	   messages.
   */
  void peon_init();//dhq: 一个节点reboot后，会走怎样的流程？
  ...
  void wait_for_writeable(MonOpRequestRef op, Context *c) {
    assert(!is_writeable());//调用本函数之前，在外围某个地方应该已经确认了不是writeable的。
    if (op)
