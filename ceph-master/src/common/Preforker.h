...
class Preforker {
  pid_t childpid;
  bool forked; //这个变量不是代表父子，而是表示做过fork
...
public:
  ...
  int prefork(std::string &err) {
    assert(!forked);
    int r = socketpair(AF_UNIX, SOCK_STREAM, 0, fd);//父子间用socket通讯 ? 以前都是pipe
  ...
  int parent_wait(std::string &err_msg) {
    ...
    int r = -1;
    std::ostringstream oss;
    int err = safe_read_exact(fd[0], &r, sizeof(r));//等待子进程写东西，啥时候写呢 ? 
    if (err == 0 && r == -1) {
      // daemonize   这个daemonize，是啥意思 ? 
      ::close(0);
      ::close(1);
      ::close(2);
      r = 0;
